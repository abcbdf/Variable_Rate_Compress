...
class ConvLSTM_encoder(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, stride, kernel_size, kernel_size_hidden, step=1, effective_step=None, bias=True):
        ...
        if isinstance(kernel_size_hidden,int):
            self.kernel_size_hidden = [kernel_size_hidden]*self.num_layers
        
        self.step = step
        self.bias = bias
        if effective_step is None:
            self.effective_step = range(step)
        self._all_layers = []
        for i in range(self.num_layers):
            ...
            cell = ConvLSTMCell(self.input_channels[i], 
                                ...
                                kernel_size_hidden = self.kernel_size_hidden[i])
        ...
        for step in range(self.step):
            ...
            for i in range(self.num_layers):
                ...
                if step == 0:
                    ...
                    (h, c) = ConvLSTMCell.init_hidden(bsize, self.hidden_channels[i], (int(height/self.stride[i]), int(width/self.stride[i])))
                    internal_state.append((h, c))
                # do forward
                name = 'cell{}'.format(i)
                (h, c) = internal_state[i]

                x, new_c = getattr(self, name)(x, h, c) # h: h(l,t-1), x_input: h(l-1,t), c: c(l,t-1);; x_output: h(l,t), new_c: c(l,t)
    ...
    def forward(self, input):
        ...
        for step in range(self.step):
            ...
            for i in range(self.num_layers):
                ...
                internal_state[i] = (x, new_c)
                if self.stride[i]>1:
                    x = F.pixel_shuffle(x, self.stride[i])
            # only record effective steps
            if step in self.effective_step:
                outputs.append(x)
